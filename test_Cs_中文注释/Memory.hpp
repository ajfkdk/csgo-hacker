...
template<class T>
dword write_memory2(handle process, dword address, T buffer)
{
	SIZE_T write_size;
	WriteProcessMemory(process, (LPVOID)address, &buffer, sizeof(T), &write_size);
	return write_size;
}
